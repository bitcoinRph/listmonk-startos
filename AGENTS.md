...
- **MCP credentials stay in the private `mcp` volume.** The sidecar reads them from `/startos-mcp/store.json`; never copy them into an interface URL, action response, README example, or log line.
